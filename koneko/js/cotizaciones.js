...
function list(){
    ...
    if(tableUser.length > 0){
        for(const i in tableUser){
            ...
            dataRow += "<td>"+varUser.product+"</td>";
            dataRow += "<td>"+varUser.value+"</td>";
            dataRow += "<td style='display: flex;'>"+"<button style='margin-right: 5px;' data-content='edit' type='button' class='btn btn-warning' onclick='openPrice("+varUser.idUser+")'>Editar Cotización<i class='fa-sharp fa-solid fa-pen-to-square'></i></button>"+"<button style='margin-left: 5px;' type='button' class='btn btn-danger' onclick='deleteItem("+varUser.idUser+")'>Eliminar cotización <i class='fa-sharp fa-solid fa-trash'></i></button>"+"</td>";
...
function openPrice(employees){
    localStorage.setItem("employees", JSON.stringify(employees));
    window.location.replace("cotizaciones-here.html");
}
